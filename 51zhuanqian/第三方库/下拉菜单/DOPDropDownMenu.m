...
//  DOPDropDownMenuDemo
//
//  Created by ZhenYu.W on 15/11/13.
//  Copyright © 2015年 unshu.com. All rights reserved.
//
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetRGBStrokeColor(context, 216.0/255, 216.0/255, 216.0/255, 1);//线条颜色
...
#define kTitleWidth 25

#define kTextColor [UIColor colorWithRed:95.0/255 green:95.0/255 blue:95.0/255 alpha:1]
#define kDetailTextColor [UIColor colorWithRed:95.0/255 green:95.0/255 blue:95.0/255 alpha:1]
#define kSeparatorColor [UIColor colorWithRed:246.0/255.0 green:246.0/255.0 blue:246.0/255.0 alpha:1]
#define kCellBgColor [UIColor colorWithRed:248/255.0 green:248/255.0 blue:248/255.0 alpha:1]
#define kTableViewCellColor [UIColor colorWithRed:216/255.0 green:216/255.0 blue:216/255.0 alpha:1]

#define IS_IPHONE_4_OR_LESS (IS_IPHONE && SCREEN_MAX_LENGTH < 568.0)
#define IS_IPHONE_5EX (IS_IPHONE && SCREEN_MAX_LENGTH == 568.0)
...
- (void)setMenuImages:(NSArray *)imageArr{
    NSMutableArray *imageMutArr = [NSMutableArray array];
    for (int i = 0; i < imageArr.count; i++) {
        CATextLayer *layer = [_titles objectAtIndex:i];
        NSString *imageStr = [imageArr objectAtIndex:i];
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        imageView.image = [UIImage imageNamed:imageStr];
//        [imageView setBackgroundColor:[UIColor redColor]];
        [imageView setCenter:CGPointMake(layer.frame.origin.x-11, self.frame.size.height/2.0)];
        [self addSubview:imageView];
        [imageMutArr addObject:imageView];
    }
    _images = [imageMutArr copy];
}
...
- (void)selectIndexPath:(DOPIndexPath *)indexPath triggerDelegate:(BOOL)trigger {
    ...
    if (indexPath.item < 0 ) {
        ...
        CGFloat sizeWidth = (size.width < (self.frame.size.width / _numOfMenu) - kTitleWidth) ? size.width : self.frame.size.width / _numOfMenu - kTitleWidth;
    ...
    ((CAShapeLayer *)[_indicators objectAtIndex:indexPath.column]).position = CGPointMake(((CATextLayer *)_titles[indexPath.column]).frame.origin.x + ((CATextLayer *)_titles[indexPath.column]).frame.size.width + 10, self.frame.size.height / 2);
}
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        //title
        CGPoint titlePosition = CGPointMake( (i * 2 + 1) * textLayerInterval  - 7 , self.frame.size.height / 2);
...
//        CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake((i + 1)*separatorLineInterval - 10, self.frame.size.height / 2)];
        ...
        //separator
        if (i != _numOfMenu - 1 && self.isShowSeparatorLine) {
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height andShowSeparatorLine:(BOOL)isShowLine{
    ...
    if (self) {
        ...
        _isShowSeparatorLine = isShowLine;
        ...
        _leftTableView.separatorColor = kTableViewCellColor;
        ...
        _rightTableView.separatorColor = kTableViewCellColor;
...
//    layer.fillColor = nil;//color.CGColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ((CAShapeLayer *)[_indicators objectAtIndex:_currentSelectedMenudIndex]).position = CGPointMake(((CATextLayer *)_titles[_currentSelectedMenudIndex]).frame.origin.x + ((CATextLayer *)_titles[_currentSelectedMenudIndex]).frame.size.width + 10, self.frame.size.height / 2);
}
...
//            title.string = [_dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:row]];
        ...
        [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
            _show = NO;
            ((CATextLayer *)[_indicators objectAtIndex:_currentSelectedMenudIndex]).position = CGPointMake( title.frame.origin.x + title.frame.size.width + 10, self.frame.size.height / 2);
            
            
        }];
        if (_images.count > 0) {
            [UIView animateWithDuration:0.25 animations:^{
                [[_images objectAtIndex:_currentSelectedMenudIndex] setCenter:CGPointMake(title.frame.origin.x - 11, self.frame.size.height/2)];
            }];
        }
        return YES;
    }
