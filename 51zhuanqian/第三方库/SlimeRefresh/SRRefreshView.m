...
{
    CGRect frame = CGRectMake(0, 0, SCREEN_WIDTH_DEVICE, height);
    self = [super initWithFrame:frame];
    if (self) {
        _slime = [[SRSlimeView alloc] initWithFrame:
                  CGRectMake(0.0f, 0.0f, SCREEN_WIDTH_DEVICE, frame.size.height)];
        _slime.startPoint = CGPointMake(SCREEN_WIDTH_DEVICE / 2, height / 2);
        ...
        _alertLabael = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 200, 20)];
//        _alertLabael.textAlignment = NSTextAlignmentCenter;
        _alertLabael.titleLabel.font = [UIFont systemFontOfSize:14.0f];
        _alertLabael.center = CGPointMake(SCREEN_WIDTH_DEVICE/2.0, height / 2 + 5);
        [_alertLabael setTitle:@"刷新成功" forState:UIControlStateNormal];
        [_alertLabael setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
//        [_alertLabael setImage:[UIImage imageNamed:@"tips_ok"] forState:UIControlStateNormal];
        _alertLabael.hidden = YES;
        [self addSubview:_alertLabael];
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (_slime.state == SRSlimeStateNormal) {
        _slime.frame = CGRectMake(0.0f, 0.0f, SCREEN_WIDTH_DEVICE, frame.size.height);
        _slime.startPoint = CGPointMake(SCREEN_WIDTH_DEVICE / 2, _dragingHeight / 2);
    }
    _refleshView.center = _slime.startPoint;
    _activityIndicatorView.center = _slime.startPoint;
}
