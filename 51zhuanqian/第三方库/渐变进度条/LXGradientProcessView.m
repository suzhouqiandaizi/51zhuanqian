...
static const CGFloat kProcessHeight = 5.f;
...
- (instancetype)initWithFrame:(CGRect)frame withColorArr:(NSArray *)colorArrEx withColorLocationArr:(NSArray *)colorLocationArrEx{
...
//        self.colorLocationArray = @[@0.1, @0.3, @0.5, @0.7, @1];
        ...
        if (colorArrEx) {
            self.colorArray = colorArrEx;
        }else{
            self.colorArray = @[(id)[RGB(253, 228, 104) CGColor],
            (id)[RGB(255, 176, 47) CGColor]];
        }
        if (colorLocationArrEx) {
            self.colorLocationArray = colorLocationArrEx;
        }else{
            self.colorLocationArray = @[@0.4, @1];
        }
...
- (void)getGradientLayer { // 进度条设置渐变色
    ...
    bgLayer.backgroundColor = RGB(255, 247, 234).CGColor;
