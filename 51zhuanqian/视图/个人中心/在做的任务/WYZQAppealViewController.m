...
@interface WYZQAppealViewController ()<UploadImageViewDelegate, MIPhotoBrowserDelegate>{
...
@end
@implementation WYZQAppealViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"申诉";
    [self addBackBtn];
    ...
    WYZQUploadImageView *upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15, 194, 90, 90)];
...
- (void)addUploadImageView:(WYZQUploadImageView *)item{
    if (uploadMutArr.count > 0 && uploadMutArr.count <= 3) {
        if (uploadMutArr.count < 3) {
            WYZQUploadImageView *upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * uploadMutArr.count, 194, 90, 90)];
            upload.delegate = self;
            [self.twoView addSubview:upload];
            [uploadMutArr addObject:upload];
        }
