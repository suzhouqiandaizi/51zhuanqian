...
@interface WYZQTaskFabuDetailViewController ()<SGPageTitleViewDelegate, SGPageContentViewDelegate>
...
@end
@implementation WYZQTaskFabuDetailViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"我发布的任务";
    [self addBackBtn];
...
- (void)requestConent{
    ...
    [[ServiceRequest sharedService] GET:[NSString stringWithFormat:@"task/published/%@/process-result", self.taskId] parameters:nil success:^(id responseObject) {
        ...
        if ([statusStr isEqualToString:@"待审核"] || [statusStr isEqualToString:@"待付款"] || [statusStr isEqualToString:@"审核失败"]){
            ...
            WYZQTaskFabuDetailProcessViewController *viewCon = [[WYZQTaskFabuDetailProcessViewController alloc] initWithNibName:@"WYZQTaskFabuDetailProcessViewController" bundle:nil];
...
- (void)setupPageView:(NSDictionary *)responseObject {
    ...
    WYZQTaskFabuDetailProcessViewController *viewCon1 = [[WYZQTaskFabuDetailProcessViewController alloc] initWithNibName:@"WYZQTaskFabuDetailProcessViewController" bundle:nil];
    viewCon1.contentArr = [responseObject objectForKey:@"historySteps"];
    [viewConMutArr addObject:viewCon1];
    [titleMutArr addObject:@"任务进程"];
    WYZQTaskFabuResultContentViewController *viewCon2 = [[WYZQTaskFabuResultContentViewController alloc] initWithNibName:@"WYZQTaskFabuResultContentViewController" bundle:nil];
