...
@interface BWRZQTaskFabuItemViewController (){
...
@end
@implementation BWRZQTaskFabuItemViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    pageNum = 1;
    pageSize = 15;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BWRZQTaskFabuItemTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"BWRZQTaskFabuItemTableViewCell" owner:self options:nil] lastObject];
    ...
    cell.chakanResult = ^(NSInteger index) {
        BWRZQTaskFabuDetailViewController *viewCon = [[BWRZQTaskFabuDetailViewController alloc] initWithNibName:@"BWRZQTaskFabuDetailViewController" bundle:nil];
        viewCon.taskId = [[self->contentMutArr objectAtIndex:index] objectForKey:@"taskNo"];
