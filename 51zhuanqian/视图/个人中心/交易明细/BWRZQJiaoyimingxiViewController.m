...
#import "MJRefresh.h"
@interface BWRZQJiaoyimingxiViewController (){
    NSInteger                  pageNum;
    NSMutableArray             *contentMutArr;
}
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@end
@implementation BWRZQJiaoyimingxiViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"交易明细";
    [self addBackBtn];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BWRZQJiaoyimingxiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"BWRZQJiaoyimingxiTableViewCell" owner:self options:nil] lastObject];
