...
#import "BWRZQSaleCoinSuccessViewController.h"
@interface BWRZQSaleCoinViewController (){
    float rate;
}
...
@end
@implementation BWRZQSaleCoinViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self addBackBtn];
    self.title = @"出售";
...
- (IBAction)surePress {
    ...
    [[ServiceRequest sharedService] POST:[NSString stringWithFormat:@"user/%@/coin-convert", user.userID] parameters:dic success:^(id responseObject) {
        [self hideHudAlert];
        BWRZQSaleCoinSuccessViewController *viewCon = [[BWRZQSaleCoinSuccessViewController alloc] initWithNibName:@"BWRZQSaleCoinSuccessViewController" bundle:nil];
