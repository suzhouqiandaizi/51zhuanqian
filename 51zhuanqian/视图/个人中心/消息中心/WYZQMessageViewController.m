...
@interface WYZQMessageViewController ()<SGPageTitleViewDelegate, SGPageContentViewDelegate>{
}
@property (nonatomic, strong) SGPageTitleView *pageTitleView;
@property (nonatomic, strong) SGPageContentView *pageContentView;
@end
@implementation WYZQMessageViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"消息";
    [self addBackBtn];
...
- (void)setupPageView {
    ...
    for (NSInteger i = 0; i < arrayID.count ; i++) {
        WYZQMessageItemViewController *viewCon = [[WYZQMessageItemViewController alloc] initWithNibName:@"WYZQMessageItemViewController" bundle:nil];
        viewCon.type = i;
