...
@interface WYZQFaBuYuLanViewController ()<MIPhotoBrowserDelegate>{
...
@end
@implementation WYZQFaBuYuLanViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"发布预览";
    [self addBackBtn];
...
- (void)showBuZhouContentView{
    ...
    for (int i = 0; i < self.buzhouArr.count; i++) {
        UILabel *lineImage;
        WYZQFaBuBuZhouObject *object = [self.buzhouArr objectAtIndex:i];
...
- (void)clickLinkPress:(id)sender{
    ...
    WYZQFaBuBuZhouObject *object = [self.buzhouArr objectAtIndex:btn.tag];
