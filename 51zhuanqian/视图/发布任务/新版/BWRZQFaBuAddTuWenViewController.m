...
@interface BWRZQFaBuAddTuWenViewController ()<UploadImageViewDelegate>{
...
@end
@implementation BWRZQFaBuAddTuWenViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"添加图文说明";
    [self addBackBtn];
    ...
    if (self.object) {
        ...
        for (int i = 0; i < taskImages.count; i++) {
            ...
            BWRZQUploadImageView *upload = [[BWRZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * i, 277, 90, 90)];
        ...
        if (taskImages.count < 3) {
            BWRZQUploadImageView *uploadEx = [[BWRZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * taskImages.count, 277, 90, 90)];
        ...
        editBool = YES;
        BWRZQUploadImageView *upload = [[BWRZQUploadImageView alloc] initItem:CGRectMake(15, 277, 90, 90)];
        upload.delegate = self;
        [self.scrollView addSubview:upload];
        [uploadMutArr addObject:upload];
    }
...
- (void)addUploadImageView:(BWRZQUploadImageView *)item{
    if (uploadMutArr.count > 0 && uploadMutArr.count <= 3) {
        if (uploadMutArr.count < 3) {
            BWRZQUploadImageView *upload = [[BWRZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * uploadMutArr.count, 277, 90, 90)];
            upload.delegate = self;
            [self.scrollView addSubview:upload];
            [uploadMutArr addObject:upload];
        }
...
- (IBAction)surePress {
    ...
    if (self.object) {
        ...
        BWRZQFaBuBuZhouObject *object = [[BWRZQFaBuBuZhouObject alloc] init];
        object.type = 1;
