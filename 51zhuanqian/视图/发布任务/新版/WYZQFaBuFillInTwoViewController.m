...
@interface WYZQFaBuFillInTwoViewController ()<UploadImageViewDelegate, FaBuFillInBuZhouTypeAlertViewDelegate, MIPhotoBrowserDelegate>{
...
@end
@implementation WYZQFaBuFillInTwoViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"发布任务";
...
- (void)requestContent:(BOOL)alert{
    ...
    [[ServiceRequest sharedService] GET:@"task/select-options" parameters:@{@"categoryName":self.taskCategoryStr} success:^(id responseObject) {
        ...
        if (self.taskId.length > 0) {
            ...
            WYZQUploadImageView *uploadEx = [[WYZQUploadImageView alloc] initItem:CGRectMake(15, originY, 90, 90)];
            uploadEx.category = 2;
...
- (void)requestDetail{
    [[ServiceRequest sharedService] GET:[NSString stringWithFormat:@"task/published/%@", self.taskId] parameters:nil success:^(id responseObject) {
        ...
        [self->buzhouMutArr addObjectsFromArray:[stepsMutArr sortedArrayUsingComparator:^NSComparisonResult(WYZQFaBuBuZhouObject *p1, WYZQFaBuBuZhouObject *p2){
        ...
        for (int i = 0; i < taskRequirementImages.count; i++) {
            ...
            WYZQUploadImageView *upload = nil;
            if (i > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
                originY = originY + 90 + 10;
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * (taskRequirementImages.count - 3), originY, 90, 90)];
            }else{
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * taskRequirementImages.count, originY, 90, 90)];
            }
            upload.category = 2;
...
- (void)jieshaoTapAction:(UIGestureRecognizer *)gesture{
    WYZQFaBuFillInTextViewViewController *viewCon = [[WYZQFaBuFillInTextViewViewController alloc] initWithNibName:@"WYZQFaBuFillInTextViewViewController" bundle:nil];
...
- (void)refreshView{
    ...
    for (int i = 0; i < renwuMutArr.count; i++) {
        WYZQUploadImageView *object = [renwuMutArr objectAtIndex:i];
        if (i > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
            originY = originY + 90 + 10;
    ...
    WYZQUploadImageView *uploadEx = [renwuMutArr lastObject];
    ...
    if (item.category == 1) {
        ...
        if (renwuMutArr.count > 0 && renwuMutArr.count < 4) {
            ...
            if (renwuMutArr.count > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
                originY = originY + 90 + 10;
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * (renwuMutArr.count - 3), originY, 90, 90)];
            }else{
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * renwuMutArr.count, originY, 90, 90)];
            }
            upload.category = 2;
            upload.delegate = self;
            upload.infoLabel.text = @"任务说明图片";
...
- (void)removeUploadImageView:(WYZQUploadImageView *)item{
    if (item.category == 1) {
        ...
        if (renwuMutArr.count > 0) {
            ...
            if (renwuMutArr.count == 3 && ((WYZQUploadImageView *)[renwuMutArr lastObject]).hasChoose) {
                WYZQUploadImageView *upload = nil;
                if (renwuMutArr.count > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
                    originY = originY + 90 + 10;
                    upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * (renwuMutArr.count - 3), originY, 90, 90)];
...
- (IBAction)buzhoutianjiaPress {
    WYZQFaBuFillInBuZhouTypeAlertView *alertView = [[WYZQFaBuFillInBuZhouTypeAlertView alloc] initItem];
...
- (void)createBuZhouType:(NSInteger)type withObject:(WYZQFaBuBuZhouObject *)objectEx{
    if (type == 2) {
        WYZQFaBuAddWangZhiViewController *viewCon = [[WYZQFaBuAddWangZhiViewController alloc] initWithNibName:@"WYZQFaBuAddWangZhiViewController" bundle:nil];
        viewCon.object = objectEx;
        viewCon.RefreshHandle = ^(WYZQFaBuBuZhouObject * _Nullable object) {
        ...
        WYZQFaBuAddFuZhiViewController *viewCon = [[WYZQFaBuAddFuZhiViewController alloc] initWithNibName:@"WYZQFaBuAddFuZhiViewController" bundle:nil];
        viewCon.object = objectEx;
        viewCon.RefreshHandle = ^(WYZQFaBuBuZhouObject * _Nullable object) {
            if ([self->buzhouMutArr containsObject:object]) {
                [self->buzhouMutArr replaceObjectAtIndex:object.index withObject:object];
        ...
        WYZQFaBuAddTuWenViewController *viewCon = [[WYZQFaBuAddTuWenViewController alloc] initWithNibName:@"WYZQFaBuAddTuWenViewController" bundle:nil];
        viewCon.object = objectEx;
        viewCon.RefreshHandle = ^(WYZQFaBuBuZhouObject * _Nullable object) {
            if ([self->buzhouMutArr containsObject:object]) {
                [self->buzhouMutArr replaceObjectAtIndex:object.index withObject:object];
...
- (void)showBuZhouContentView{
    ...
    for (int i = 0; i < buzhouMutArr.count; i++) {
        WYZQFaBuBuZhouObject *object = [buzhouMutArr objectAtIndex:i];
        object.index = i;
        if (!object.isCreated) {
...
- (void)deleteBuzhou:(id)sender{
    ...
    WYZQFaBuBuZhouObject *object = [buzhouMutArr objectAtIndex:btn.tag];
...
- (IBAction)yulanPress {
    ...
    WYZQFaBuYuLanViewController *viewCon = [[WYZQFaBuYuLanViewController alloc] initWithNibName:@"WYZQFaBuYuLanViewController" bundle:nil];
    if (self.taskId.length > 0 && !self.againCreate) {
        viewCon.isEdit = YES;
    }
...
- (IBAction)surePress {
    ...
    if (self.taskId.length > 0 && !self.againCreate) {
        ...
        [[ServiceRequest sharedService] POSTJSON:@"task" parameters:[self getParameterDic] success:^(id responseObject) {
            [self hideHudAlert];
            WYZQFaBuSuccessViewController *viewCon = [[WYZQFaBuSuccessViewController alloc] initWithNibName:@"WYZQFaBuSuccessViewController" bundle:nil];
...
- (void)pushImage:(NSInteger)type withImage:(WYZQUploadImageView *)item{
...
- (void)stepTapAction:(UIGestureRecognizer *)gesture{
    ...
    WYZQFaBuBuZhouObject *object = [buzhouMutArr objectAtIndex:imageView.tag];
...
- (UIImage *)photoBrowser:(MIPhotoBrowser *)photoBrowser placeholderImageForIndex:(NSInteger)index{
    WYZQFaBuBuZhouObject *object = [buzhouMutArr objectAtIndex:photoBrowser.currentImgaeView.tag];
