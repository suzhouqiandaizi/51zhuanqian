...
@interface BWRZQFaBuDetailTwoViewController ()<MIPhotoBrowserDelegate>{
    NSArray        *buzhouArr;
...
@end
@implementation BWRZQFaBuDetailTwoViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self addBackBtn];
    self.title = @"任务详情";
...
- (void)requestContent:(BOOL)alert{
    ...
    [[ServiceRequest sharedService] GET:@"task/select-options" parameters:@{@"categoryName":self.taskCategoryStr} success:^(id responseObject) {
        ...
        [[ServiceRequest sharedService] GET:[NSString stringWithFormat:@"task/published/%@", self.taskId] parameters:nil success:^(id responseObject) {
            ...
            self->buzhouArr = [stepsMutArr sortedArrayUsingComparator:^NSComparisonResult(BWRZQFaBuBuZhouObject *p1, BWRZQFaBuBuZhouObject *p2){
...
- (void)showBuZhouContentView{
    ...
    for (int i = 0; i < buzhouArr.count; i++) {
        BWRZQFaBuBuZhouObject *object = [buzhouArr objectAtIndex:i];
...
- (void)stepTapAction:(UIGestureRecognizer *)gesture{
    ...
    BWRZQFaBuBuZhouObject *object = [buzhouArr objectAtIndex:imageView.tag];
...
- (UIImage *)photoBrowser:(MIPhotoBrowser *)photoBrowser placeholderImageForIndex:(NSInteger)index{
    if (photoBrowser.tagMut == 2) {
        ...
        BWRZQFaBuBuZhouObject *object = [buzhouArr objectAtIndex:photoBrowser.currentImgaeView.tag];
