...
@interface WYZQFaBuFillInViewController ()<UploadImageViewDelegate>{
...
@end
@implementation WYZQFaBuFillInViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"发布任务";
...
- (void)requestContent:(BOOL)alert{
    ...
    [[ServiceRequest sharedService] GET:@"task/select-options" parameters:@{@"categoryName":self.taskCategoryStr} success:^(id responseObject) {
        ...
        if (self.taskId.length > 0) {
            ...
            WYZQUploadImageView *upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15, originY, 90, 90)];
            upload.category = 1;
    ...
    [[ServiceRequest sharedService] GET:[NSString stringWithFormat:@"task/published/%@", self.taskId] parameters:nil success:^(id responseObject) {
        ...
        for (int i = 0; i < taskStepImages.count; i++) {
            ...
            if (i > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
                ...
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * (taskStepImages.count - 3), originY, 90, 90)];
            }else{
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * taskStepImages.count, originY, 90, 90)];
            }
            upload.category = 1;
            upload.delegate = self;
            upload.infoLabel.text = @"步骤说明图片";
...
- (void)refreshView{
    ...
    for (int i = 0; i < buzhouMutArr.count; i++) {
        WYZQUploadImageView *object = [buzhouMutArr objectAtIndex:i];
        if (i > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
            originY = originY + 90 + 10;
    ...
    WYZQUploadImageView *upload = [buzhouMutArr lastObject];
    ...
    if (item.category == 1) {
        ...
        if (buzhouMutArr.count > 0 && buzhouMutArr.count < 4) {
            ...
            if (buzhouMutArr.count > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
                originY = originY + 90 + 10;
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * (buzhouMutArr.count - 3), originY, 90, 90)];
            }else{
                upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * buzhouMutArr.count, originY, 90, 90)];
            }
            upload.category = 1;
            upload.delegate = self;
            upload.infoLabel.text = @"步骤说明图片";
...
- (void)removeUploadImageView:(WYZQUploadImageView *)item{
    if (item.category == 1) {
        ...
        if (buzhouMutArr.count > 0) {
            ...
            if (buzhouMutArr.count == 3 && ((WYZQUploadImageView *)[buzhouMutArr lastObject]).hasChoose) {
                WYZQUploadImageView *upload = nil;
                if (buzhouMutArr.count > 2 && SCREEN_WIDTH_DEVICE - ((90 + 10) * 3) - 30 < 90) {
                    originY = originY + 90 + 10;
                    upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * (buzhouMutArr.count - 3), originY, 90, 90)];
