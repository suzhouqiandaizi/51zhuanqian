...
@interface BWRZQSingleTaskViewController ()<TaskClassifyViewDelegate>{
    NSMutableArray                  *contentMutArr;
    NSInteger                       pageNum;
    NSInteger                       pageSize;
    BWRZQTaskClassifyView                *alertView;
    NSArray                         *chooseArr;
    NSString                        *chooseStr;
...
@end
@implementation BWRZQSingleTaskViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    if (self.titleStr) {
...
- (IBAction)onePress {
    if (self.oneBtn.selected) {
        ...
        if (!alertView) {
            alertView = [[BWRZQTaskClassifyView alloc] initItem];
