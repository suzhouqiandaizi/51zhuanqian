...
@interface WYZQSubmitTaskViewController ()<UploadImageViewDelegate, UIScrollViewDelegate, MIPhotoBrowserDelegate>{
...
@end
@implementation WYZQSubmitTaskViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"提交任务结果";
    [self addBackBtn];
...
- (void)showContent{
    ...
    WYZQUploadImageView *upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15, 44, 90, 90)];
...
- (void)addUploadImageView:(WYZQUploadImageView *)item{
    if (uploadMutArr.count > 0 && uploadMutArr.count <= 3) {
        if (uploadMutArr.count < 3) {
            WYZQUploadImageView *upload = [[WYZQUploadImageView alloc] initItem:CGRectMake(15 + (90 + 10) * uploadMutArr.count, 44, 90, 90)];
            upload.delegate = self;
            [self.twoView addSubview:upload];
            [uploadMutArr addObject:upload];
        }
...
- (IBAction)surePress {
    ...
    [[ServiceRequest sharedService] POSTJSON:[NSString stringWithFormat:@"task-order/%@/%@/submit", user.userID, self.task.orderId] parameters:dic success:^(id responseObject) {
        ...
        if ([DEFAULTS boolForKey:@"NEVERSHOWSUBMITALERT"]) {
            WYZQSubmitSuccessInfoView *alertView = [[WYZQSubmitSuccessInfoView alloc] initItem];
            ...
            WYZQSubmitAlertView *alertView = [[WYZQSubmitAlertView alloc] initItem];
