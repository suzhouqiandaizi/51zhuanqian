...
@interface BWRZQTaskDetailTwoViewController ()<MIPhotoBrowserDelegate>{
...
@end
@implementation BWRZQTaskDetailTwoViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"任务详情";
    [self addBackBtn];
    if (![DEFAULTS boolForKey:@"NEVERSHOWTASKDETAILALERT"]) {
        BWRZQTaskDetailTwoAlertView *alertView = [[BWRZQTaskDetailTwoAlertView alloc] initItem];
...
- (void)lineCilicAction:(UIGestureRecognizer *)gesture{
    ...
    if (taskObject.zidailiulangqi) {
        ...
        if ([taskObject.taskType isEqualToString:@"阅读任务"]) {
            BWRZQBaiduTaskReadViewController *viewCon = [[BWRZQBaiduTaskReadViewController alloc] initWithNibName:@"BWRZQBaiduTaskReadViewController" bundle:nil];
