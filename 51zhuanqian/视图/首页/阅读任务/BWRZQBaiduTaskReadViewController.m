...
@interface BWRZQBaiduTaskReadViewController ()<UIScrollViewDelegate,UINavigationControllerDelegate,WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler>{
    NSInteger       count;
    NSInteger       currentReadTimeNum;
    NJKWebViewProgressView *_progressWebView;
}
...
@end
@implementation BWRZQBaiduTaskReadViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self addBackBtn];
    currentReadTimeNum = 0;
    [self.webContentView setFrame:CGRectMake(0, 64 + IS_iPhoneX_Top, SCREEN_WIDTH_DEVICE, SCREEN_HEIGHT_DEVICE - IS_iPhoneX_Top - 64)];
    [self.floatView setFrame:CGRectMake(SCREEN_WIDTH_DEVICE - 66 - 8, SCREEN_HEIGHT_DEVICE - 66 - IS_iPhoneX_Bottom - 150, 66, 66)];
    ...
    _progressWebView = [[NJKWebViewProgressView alloc] initWithFrame:barFrame];
    _progressWebView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    _progressWebView.progressColor = ThemeColor;
    self.floatView.hidden = YES;
    [self createWebView:self.urlStr];
}
- (void)viewWillAppear:(BOOL)animated
{
    [super viewWillAppear:animated];
    [self.navigationController.navigationBar addSubview:_progressWebView];
}
- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [_progressWebView removeFromSuperview];
}
...
- (void)dealloc
{
    [[ServiceRequest sharedService] cancelDataTaskForKey:[NSString stringWithFormat:@"task/read-records/%@", self.taskId]];
...
- (void)startJiShi{
    ...
    dispatch_source_set_event_handler(self.timer, ^{
        ...
        if (self->count > 0 && self.progressView.progress < 1.0f) {
            ...
            float percentage = self->currentReadTimeNum/(float)self.readTimeNum;
            NSLog(@"++++++=%f", percentage);
            ...
            if (self.progressView.progress >= 1) {
                ...
                [dic setObject:[NSNumber numberWithInteger:self.readTimeNum] forKey:@"readSeconds"];
                [[ServiceRequest sharedService] POST:[NSString stringWithFormat:@"task/read-records/%@", self.taskId] parameters:dic success:^(id responseObject) {
...
- (void)createWebView:(NSString *)urlStr
{
    ...
    self.webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH_DEVICE, self.webContentView.frame.size.height) configuration:config];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"estimatedProgress"]){
        if (object == self.webView){
            ...
            [_progressWebView setProgress:self.webView.estimatedProgress animated:YES];
...
- (void)webView:(WKWebView *)webView didFailProvisionalNavigation:(WKNavigation *)navigation withError:(NSError *)error {
    NSLog(@"加载失败");
    [_progressWebView setProgress:0 animated:NO];
}
...
-(void)popSnapShotViewWithPanGestureDistance:(CGFloat)distance{
    ...
    CGPoint currentSnapshotViewCenter = CGPointMake(SCREEN_WIDTH_DEVICE/2, SCREEN_HEIGHT_DEVICE/2);
    currentSnapshotViewCenter.x += distance;
    CGPoint prevSnapshotViewCenter = CGPointMake(SCREEN_WIDTH_DEVICE/2, SCREEN_HEIGHT_DEVICE/2);
    prevSnapshotViewCenter.x -= (SCREEN_WIDTH_DEVICE - distance)*60/SCREEN_WIDTH_DEVICE;
    self.currentSnapShotView.center = currentSnapshotViewCenter;
    self.prevSnapShotView.center = prevSnapshotViewCenter;
    self.swipingBackgoundView.alpha = (SCREEN_WIDTH_DEVICE - distance)/SCREEN_WIDTH_DEVICE;
}
-(void)endPopSnapShotView{
    ...
    if (self.currentSnapShotView.center.x >= SCREEN_WIDTH_DEVICE) {
        [UIView animateWithDuration:0.2 animations:^{
            [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
            self.currentSnapShotView.center = CGPointMake(SCREEN_WIDTH_DEVICE*3/2, SCREEN_HEIGHT_DEVICE/2);
            self.prevSnapShotView.center = CGPointMake(SCREEN_WIDTH_DEVICE/2, SCREEN_HEIGHT_DEVICE/2);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            [UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
            self.currentSnapShotView.center = CGPointMake(SCREEN_WIDTH_DEVICE/2, SCREEN_HEIGHT_DEVICE/2);
            self.prevSnapShotView.center = CGPointMake(SCREEN_WIDTH_DEVICE/2-60, SCREEN_HEIGHT_DEVICE/2);
            self.prevSnapShotView.alpha = 1;
        }completion:^(BOOL finished) {
            [self.prevSnapShotView removeFromSuperview];
            [self.swipingBackgoundView removeFromSuperview];
