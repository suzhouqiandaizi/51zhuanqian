...
extern NSString *const WZYBannerViewDidClick;    //监听点击弹窗的事件

@interface WZYForeNotification : NSObject
...
static NSString *const WZYBannerViewTimeText = @"现在";    //默认弹窗时间 default banner time
...
⭐2.监听点击弹窗的事件、用户点击以后可以取到推送内容、通过自定义推送中的 extra 字段处理自己的逻辑。如：
    add observer for "WZYBannerViewDidClick", handle extra key/value
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(WZYBannerViewDidClick:) name:WZYBannerViewDidClick object:nil];
    -(void)WZYBannerViewDidClick:(NSNotification*)noti{
        if(noti[@"key1" == @"跳转页面1"]){
            //jump to page 1
        }
    }
...
⭐4.使用系统自带声音，默认的推送三全音 id 是 1312
    soundID: is ios system sound id, default push notification sound "Tritone" is 1312
     ...
     1070	ct-busy.caf	ct-busy.caf	AudioTonWZYusy	There was no category for this sound before 4.0.
