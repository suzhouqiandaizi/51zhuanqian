...
@interface WZYPickerView () <UIPickerViewDelegate, UIPickerViewDataSource>
...
@property (copy) void (^onDismissCompletion)(NSString *selectStr, NSInteger row, BOOL doneBool);
@property (copy) NSString *(^objectToStringConverter)(id object);

@end


@implementation WZYPickerView

#pragma mark - Singleton

+ (WZYPickerView*)sharedView {
  static dispatch_once_t once;
  static WZYPickerView *sharedView;
...
-(void)dismiss{
 [WZYPickerView dismissWithCompletion:self.onDismissCompletion];
}
...
-(void)cancel{
    ...
    [UIView animateWithDuration:0.3
                     ...
                     animations:^{
                         ...
                         [WZYPickerView removePickerView];
                     }];
}
...
-(void)setPickerHidden: (BOOL)hidden
              callBack: (void(^)(NSString *selectStr, NSInteger row, BOOL doneBool))callBack; {
  ...
  [UIView animateWithDuration:0.3
                   ...
                   animations:^{
                     ...
                     if(completed && hidden){
                       [WZYPickerView removePickerView];
                       callBack([self selectedObject], [self.pickerView selectedRowInComponent:0], hidden);
                     }
                   }];
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  _pickerViewContainerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH_DEVICE, SCREEN_HEIGHT_DEVICE)];
  [_pickerViewContainerView setBackgroundColor: [UIColor colorWithRed:0.412 green:0.412 blue:0.412 alpha:0.6]];
  [self addSubview:_pickerViewContainerView];
  
  //PickerView Container with top bar
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, _pickerViewContainerView.bounds.size.height - 240.0, SCREEN_WIDTH_DEVICE, 240.0)];
    
    UIButton *colseBtn = [[UIButton alloc] initWithFrame:view.bounds];
    [colseBtn addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
    [_pickerViewContainerView addSubview:colseBtn];
...
//  [_pickerViewBarButtonItem setTintColor:buttonTextColor];
  ...
  //[_pickerViewBarButtonItem setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys: [UIFont fontWithName:@"Helvetica-Neue" size:23.0], UITextAttributeFont,nil] forState:UIControlStateNormal];
   ...
   _pickerDoneButton = [[UIButton alloc] initWithFrame:CGRectMake(_pickerContainerView.frame.size.width - 60.0, 0.0, 60.0, 40.0)];
   [_pickerDoneButton setTitle:@"确定" forState:UIControlStateNormal];
    _pickerDoneButton.titleLabel.font = [UIFont systemFontOfSize:15.0f];
    [_pickerDoneButton setTitleColor:ThemeColor forState:UIControlStateNormal];
   ...
   [_pickerDoneButton addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
    ...
    _pickerCancelButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0.0, 60.0, 40.0)];
    [_pickerCancelButton setTitle:@"取消" forState:UIControlStateNormal];
    _pickerCancelButton.titleLabel.font = [UIFont systemFontOfSize:15.0f];
    [_pickerCancelButton setTitleColor:TextGrayColor forState:UIControlStateNormal];
    [_pickerContainerView addSubview:_pickerCancelButton];
    [_pickerCancelButton addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
  
  
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 30.0, SCREEN_WIDTH_DEVICE, 200.0)];
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
  if (self.objectToStringConverter == nil) {
    self.onDismissCompletion ([_pickerViewArray objectAtIndex:row], row, NO);
  } else{
    self.onDismissCompletion (self.objectToStringConverter ([self selectedObject]), row, NO);
  }
}
